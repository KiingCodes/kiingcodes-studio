...
export const Navbar = () => {
  ...
  return (
    <motion.nav
      ...
      className={`fixed top-0 left-0 right-0 z-50 transition-all duration-300 bg-background/60 backdrop-blur-xl border-b border-border/30 ${
        isScrolled ? "shadow-sm bg-background/80 border-border" : ""
      }`}
    >
            ...
            {navLinks.map((link) =>
              link.children ? (
                <div key={link.label} className="relative group">
                  <span
                    className={`text-sm font-medium transition-colors duration-200 flex items-center gap-1 cursor-default ${
        ...
        {isMobileMenuOpen && (
          ...
          >
            ...
            {user && (
              <Link
                to="/reports"
                className="block py-3 px-4 text-muted-foreground hover:text-foreground"
                onClick={() => setIsMobileMenuOpen(false)}
              >
                Reports
              </Link>
            ...
            {roleLinks.map(({ href, label }) => (
              <Link
                key={href}
                to={href}
                className={`block py-3 px-4 font-semibold ${isRouteActive(href) ? "text-primary bg-primary/5" : "text-muted-foreground hover:text-foreground"}`}
                onClick={() => setIsMobileMenuOpen(false)}
              >
                {label}
              </Link>
